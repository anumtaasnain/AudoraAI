...
exports.createEvent = async (req, res, next) => {
  try {
    const event = await Event.create({ ...req.body, organizerId: req.user._id, status: 'pending_admin' });
...
exports.registerForEvent = async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      ...
      data: { relevanceScore: score, relevanceStatus: status, status: 'pending_admin' },
    });
  } catch (err) {
    next(err);
  }
