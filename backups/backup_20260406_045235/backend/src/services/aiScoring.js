...
const getCompletenessScore = (profile) => {
  const fields = [
    ...
    profile.company, profile.jobTitle, profile.industry,
    profile.companySize, profile.eventInterest, profile.hearAboutUs,
  ];
 ...
 */
const calculateRelevanceScore = (profile, event) => {
  const eventType = event.eventType || 'ai-ml';

  const industryScore  = (INDUSTRY_EVENT_MATCH[profile.industry]  || INDUSTRY_EVENT_MATCH.other)[eventType];
  const interestScore  = (INTEREST_EVENT_MATCH[profile.eventInterest] || INTEREST_EVENT_MATCH['ai-ml'])[eventType];
  ...
  // Weighted average (as per README)
  const score =
    industryScore  * 0.35 +
    seniorityScore * 0.25 +
    sizeScore      * 0.15 +
...
const calculateMatchScore = (registration, profile, sponsorProfile) => {
  ...
  if (sponsorProfile && sponsorProfile.targetIndustries && sponsorProfile.targetIndustries.length > 0) {
    industryMatchScore = sponsorProfile.targetIndustries.includes(profile.industry) ? 100 : 30;
  }
