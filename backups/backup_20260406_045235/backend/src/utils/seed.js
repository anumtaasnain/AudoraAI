...
const { calculateRelevanceScore, getRelevanceStatus, calculateMatchScore, getLeadQuality, calculateConversionProbability, estimateDealValue } = require('../services/aiScoring');
...
const seed = async () => {
  ...
  const organizer = await User.create({
    email: 'organizer@audienceai.com',
  ...
  for (const a of attendeesData) {
    ...
    const profile = await AttendeeProfile.create({
      ...
      company: a.company, jobTitle: a.jobTitle, industry: a.industry,
      companySize: a.companySize, eventInterest: a.eventInterest,
      hearAboutUs: 'referral',
    });

    const score  = calculateRelevanceScore(profile, event);
    const status = getRelevanceStatus(score);

    await EventRegistration.create({
      eventId: event._id, userId: user._id,
      relevanceScore: score, relevanceStatus: status,
      isQualifiedLead: score >= 80,
    });

    await ActivityLog.create({
      eventId: event._id, userId: user._id,
      type:  score >= 80 ? 'lead_identified' : 'attendee_flagged',
      title: score >= 80 ? 'High-quality lead identified' : 'Low relevance attendee flagged',
      description: `${a.firstName} ${a.lastName} (${a.jobTitle}, ${a.company}) registered for ${event.title}`,
    });
    console.log(`  👤 ${a.firstName} ${a.lastName} → score: ${score} [${status}]`);
  }
  ...
  console.log('   Organizer → organizer@audienceai.com / password123');
  console.log('   Sponsor   → sponsor@dataflow.io      / password123');
  console.log('   Admin     → admin@audienceai.com     / password123');
