...
import { LayoutDashboard, Users, BarChart3, DollarSign, Settings, Bell, Search, Menu, Target, LogOut, Calendar, FileCheck, User, Tags } from 'lucide-react';
...
export default function DashboardLayout() {
  ...
  if (user?.role === 'attendee') {
    ...
    if (user?.role === 'admin' || user?.role === 'organizer') {
      ...
      navItems.push({ path: '/dashboard/sponsorships', label: 'Sponsorships', icon: DollarSign });
    }
    if (user?.role === 'sponsor') {
       navItems.push({ path: '/dashboard/sponsorships', label: 'Incoming Requests', icon: DollarSign });
    }
    if (user?.role === 'admin') {
      navItems.push({ path: '/dashboard/categories', label: 'Categories', icon: Tags });
    }
  ...
  const SidebarContent = () => (
    <div className="flex flex-col flex-1 min-h-0">
      {/* Logo */}
      <div className="flex items-center gap-2 px-6 py-5 border-b cursor-pointer" onClick={() => navigate('/')}>
