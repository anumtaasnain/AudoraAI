...
import { eventService, categoryService } from '../../lib/services';
import { Calendar, MapPin, CheckCircle2, XCircle, Send, AlertCircle, PlusCircle, Building, Tag } from 'lucide-react';
...
export default function EventsPage() {
  ...
  // Fetch categories (interests & industries)
  useEffect(() => {
  ...
  const onCreateEvent = async (data: EventForm) => {
    try {
      // Clean up optional fields - convert empty strings to undefined so they're omitted
      const payload = {
        ...data,
        ...
        <Card className="p-6 mb-8 border-2 shadow-lg">
          ...
          <form onSubmit={handleSubmit(onCreateEvent)} className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              ...
              {/* Interest Category (required) */}
              <div className="space-y-2">
                <Label>Event Interest <span className="text-red-500">*</span></Label>
                <select
                  className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm"
                  {...register('interestId', { required: true })}
                  required
                >
