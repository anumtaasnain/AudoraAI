...
import { sponsorService, userService } from '../../lib/services';
import { useAuth } from '../../context/AuthContext';
import { Search, Flame, ThermometerSun, Snowflake, DollarSign, Target, Briefcase, Mail, Building2, ExternalLink, Zap, User, Tags } from 'lucide-react';
...
export default function SponsorPage() {
  ...
  const fetchSponsors = useCallback(async () => {
    setLoading(true);
    try {
      const res = await userService.getSponsors();
  ...
  useEffect(() => { 
    if (user?.role === 'sponsor') fetchAll(); 
    else fetchSponsors();
  }, [fetchAll, fetchSponsors, user]);
  ...
  if (user?.role !== 'sponsor') {
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Sponsor Directory</h1>
          <p className="text-gray-600">List of all registered sponsors on the platform</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sponsors.map((s: any) => (
            <Card key={s.id} className="p-6 border-2 hover:shadow-lg transition-all bg-white rounded-3xl overflow-hidden relative">
               <div className="absolute top-0 right-0 p-4 opacity-5">
                  <Building2 className="w-16 h-16" />
               </div>
               <div className="flex items-center gap-4 mb-6">
                  <div className="w-14 h-14 bg-gradient-to-br from-blue-600 to-purple-600 rounded-2xl flex items-center justify-center text-white text-xl font-bold">
                     {s.email[0].toUpperCase()}
                  </div>
                  <div>
                     <h3 className="text-lg font-bold text-gray-900">{s.profile?.company || 'Organization TBA'}</h3>
                     <p className="text-sm text-gray-500">{s.email}</p>
                  </div>
               </div>
               <div className="space-y-3 mb-6">
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                     <User className="w-4 h-4" /> <span>{s.profile?.firstName} {s.profile?.lastName || 'Representative'}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                     <Building2 className="w-4 h-4" /> <span>{s.profile?.companySize || 'Unknown size'}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-blue-600 font-medium">
                     <Tags className="w-4 h-4" /> <span>{s.profile?.industryIds?.[0]?.name || 'Multiple Industries'}</span>
                  </div>
               </div>
               <Button variant="outline" className="w-full rounded-xl" onClick={() => window.location.href = `mailto:${s.email}`}>
                  Contact Partner
               </Button>
            </Card>
          ))}
          {sponsors.length === 0 && <p className="text-gray-500 col-span-full text-center py-20">No sponsors registered yet.</p>}
        </div>
      </div>
    );
  }
